...
const ToastProvider: FC<{ children: React.ReactNode }> = (props) => {
  ...
  const { shareCharacterInfo, copyLink, shortUrl, closeShare, handleImportCharacter } = useCharacterCrud()
  const [importCharacter, setImportCharacter] = useState<string>('')
  ...
  return (
    <div>
        ...
        shareCharacterInfo.type === 'share' && <div className="fixed inset-0 z-30 flex flex-col items-center justify-center">
          ...
          <div onClick={eventStopPropagation} className="relative w-[311px] h-[280px] leading-[1]">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-end pb-[20px]">
              ...
              <div className="text-[#11111188] text-[12px]">（正式版可以输入密钥唤醒角色开始故事）</div>
              <div style={{ backgroundImage: `url(${$img('share-item-bg')})` }}
                className="w-[251px] h-[40px] mt-[12px] flex items-center gap-[6px] bg-cover bg-center px-[8px]">
      ...
      {
        shareCharacterInfo.type === 'import' && <div className="fixed inset-0 z-30 flex flex-col items-center justify-center">
          <div onClick={closeShare} className="absolute inset-0 bg-[#000000B2] backdrop-blur-[5px]"></div>
          <div onClick={eventStopPropagation} className="relative w-[311px] h-[280px] leading-[1]">
            <img className="w-full h-full" src={$img('bg-invite')} alt="bg-invite" />
            <div className="absolute inset-0 flex flex-col items-center justify-end pb-[20px]">
              <div className="text-[#111] text-[24px]">输入6位角色密令</div>
              <div className="text-[#11111188] text-[12px]">（输入好友分享给你的密令，召唤同款角色）</div>
              <div className="w-[263px] mt-[24px] mb-[30px] pb-[8px] border-b-[1px] border-[#111111]">
                <input className="text-[48px] text-center w-full placeholder:text-[#1111114C] focus-visible:outline-none"
                  maxLength={6} type="text" placeholder="点击输入" value={importCharacter}
                  onChange={(e) => setImportCharacter(e.target.value)} />
              </div>
              <img onClick={() => { handleImportCharacter(importCharacter) }} className="w-[168px] h-[40px] mb-[20px]" src={$img('btn-import-certain')} alt="btn-import-certain" />
            </div>
          </div>
          <img onClick={closeShare} className="relative w-[32px] h-[32px] mt-[16px]" src={$img('icon-cancel')} alt="icon-cancel" />
        </div>
